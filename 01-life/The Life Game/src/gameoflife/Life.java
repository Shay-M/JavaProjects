...
public final class Life {
    ...
    public Life(final GameRules<Boolean> gameRules, final FrameWriter frameWriter, final int width, final int height, final int iterations, final int numThreads, final double initialPopulation) {
    ...
    public void start() {
        ...
        m_currentGenerationGrid = new Grid<>(m_height, m_width, () -> secureRandom.nextDouble() > m_initialPopulation);
        m_nextGenerationGrid = new Grid<>(m_height, m_width, () -> Boolean.FALSE);
    ...
    private void simulateIterationsGenerations() {
        ...
        final var threads = createThreads(cyclicBarrier);
        threads.forEach(Thread::start);
        waitsForThisThreadToTerminate(threads);

    }

    private ArrayList<Thread> createThreads(final CyclicBarrier cyclicBarrier) {
        final var threads = new ArrayList<Thread>(m_numThreads);
        final int sizeGridThread = m_width / m_numThreads;


        // m_numThreads - 1 Main thread also helps with calculation
        for (int i = 0; i < m_numThreads; ++i) { //-1
        ...
        // Main thread also helps with calculation
        // var startRow = m_width - sizeGridThread; // last thread may have more rows to handle.
        // simulateSegmentOfGrid(startRow, m_width, cyclicBarrier);
        // thread.join();
    ...
    private void waitsForThisThreadToTerminate(final ArrayList<Thread> threads) {
